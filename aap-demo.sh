...
check_mkcert_ca() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        if [ -f "$CAROOT/rootCA.pem" ]; then
            ...
            if [ -f /etc/ssl/certs/mkcert* ] || \
               [ -f /usr/local/share/ca-certificates/mkcert* ] || \
               [ -f /etc/pki/ca-trust/source/anchors/mkcert* ]; then
                CA_INSTALLED=true
...
cmd_diagnose() {
    ...
    _check_warn() { printf "  \033[33m⚠\033[0m %s\n" "$1"; warnings=$((warnings + 1)); }
...
_run_atf() {
    ...
    (umask 077; cat > "${artifacts_dir}/atf_installer_inventory" <<INVEOF
